import { EDZ_REGISTER_NEW_CHILD_URL, REGISTER_NEW_CHILD_URL, REGISTER_USING_EMAIL_URL } from "../../../constants/ApiPaths";
...
export const RegisterNewGmailChild = async (bodydata: any, callBack: any) => {
  const url = REGISTER_USING_EMAIL_URL;
  console.log(url, "REGISTER_USING_EMAIL_URL_url...........................");
  const token = await AsyncStorage.getObject("@auth_Token");
  const requestOptions = {
    method: "POST",
    headers: {
  ...
  try {
    // const response = await axios.post(url,bodydata, requestOptions);
    const response = await fetch(url, requestOptions);
    console.log(response, "REGISTER_USING_EMAIL_URL_response..........");
    // const Data = await response.data;
    const Data = await response.json();
    console.log(Data,"data@@@@@");
    ...
    console.log(error, "REGISTER_USING_EMAIL_URL_error.......");
    throw error;
  }
};